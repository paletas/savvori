...
- Category classifier (Phase 4): `CategoryClassifier` (nightly `CategoryClassifierJob`, or "Run classifier now") predicts categories for products that have NONE by k-NN (k=7, similarity-weighted vote) over stored embeddings of categorised products; existing labels are never touched and the classifier never learns from its own decisions. Confidence >= 0.85 is assigned, 0.5 to 0.85 goes to the review queue, lower stays uncategorised. Raw store-category strings (for example Celeiro's `alimentacao`) are decided once as a whole when at least `MinStringSupport` uncategorised products carry them and agreement reaches the auto-assign level; the decision is cached in `CategoryStringDecisions` and reused, and heterogeneous strings are marked Mixed so products are decided one by one. Every decision is a `CategorySuggestions` row (method, confidence, model, digest, timestamp, previous category) and is undoable. `Model:Categories:DryRun` defaults to true. It decides nothing while the flag is off or the breaker is not closed, so the rule-based `CategoryMapper` stays the degraded-mode path. The taxonomy v2 migration itself is NOT done: see `docs/TAXONOMY_V2.md`.
- Taxonomy v2 (`Scraping/TaxonomyV2*.cs`, `TaxonomyMigrationService`): explicit, reversible admin action. `GET /api/admin/taxonomy/plan` is a dry run; `POST /apply` seeds the 12 aisles / 87 categories (reusing rows with shared slugs), relabels products through the approved v1->v2 mapping (deterministic keyword rules; anything unmatched is left uncategorised for the classifier), keeps `Product.LegacyCategoryId`, and backfills tags (`ProductTags`: bio, sem-lactose, sem-gluten, vegan, sem-acucar; rules only, never the model); `POST /revert` restores v1 labels and names/parents but keeps labels you set by hand. Once active, `/api/categories` shows the v2 tree and the scraper translates the rule-based mapper's v1 slug to a v2 category using the product name. Tags for new products are added after each scrape.
...
- `GET /api/admin/categorisation/summary` | `review?filter=suggested|applied` | `strings` — dry-run state, review queue, whole-string proposals.
