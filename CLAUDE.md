...
- Optional, feature-flagged (`Model:Enabled`, default off) and never on a request or scrape path: a remote Ollama model embeds product text, proposes cross-chain matches and category predictions in background Quartz jobs. Everything degrades to the deterministic behaviour when the model is down (circuit breaker + durable `ModelJobs` queue). The model only ever suggests: every merge or category it proposes is applied by a person (one at a time or in bulk, undoable); there is no dry-run switch. The one exception is multilingual search aliases (`ProductSearchAlias`, a `Translate` job): they are written without review because they only widen `GET /api/products?search=` and never affect matching, merging or categories. Search itself is bilingual without the model: a curated EN/PT glossary (`Services/SearchGlossary.cs`, applied by `ProductSearchQuery`) makes "rice" and "arroz" return the same products and orders results by relevance; aliases only cover the long tail beyond it. Design, rules and per-phase reports: `docs/MODEL_MATCHING_PLAN.md`; the category tree is taxonomy v2, applied automatically at startup: `docs/TAXONOMY_V2.md`.
